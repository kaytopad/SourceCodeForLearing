...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	...
	sHitRect stageRact;
	stageRact.worldLX = 0;
	stageRact.worldLY = -64;
	stageRact.worldRX = 64 * 120;
	stageRact.worldRY = 64 * 20;
	scrollManagerInit(1920,1080,stageRact);
	int moveFlag = 0;
	...
	while (ProcessMessage() == 0 && CheckHitKey(KEY_INPUT_ESCAPE) == 0)
	{
		...
		scrollUpdate(deltaTime,playerRect.worldLX);
